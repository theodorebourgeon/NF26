...
SQLREQUEST = session.prepare(
    f"""INSERT INTO {KEYSPACE}.{TABLE} (station, year, month, day, hour, minute, tmpf, relh, sknt, drct, alti, feel)
    values (?,?,?,?,?,?,?,?,?,?,?,?)""")
...
with open(FILE) as csvfile:
    ...
    for row in tqdm(reader):
        ...
        if COUNT<BATCH_SIZE:
            batch.add(SQLREQUEST, (station, valid[0], valid[1], valid[2], valid[3], valid[4], tmpf, relh, sknt, drct, alti, feel))
            COUNT += 1
        else:
            COUNT = 0
